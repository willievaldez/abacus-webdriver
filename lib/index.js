...
// launche sauce connect proxy if config specifies it
const proxyConnection = new Promise((resolve, reject) => {
...
const promises = [
  filterScenarios(), // parses all '*.feature' files and filters them by the given scenario tags
    gatherHooks(), // gethers all hooks in the specified hooks folder
    proxyConnection // launches sauce connect proxy if specified
];
            ...
            fs.writeFile(pathName, JSON.stringify(pack, null, 4), 'utf8', (err) => {
                ...
                childDriver.on('message', (m) => {
                    ...
                    if (conglomeratedReport.length === scenarioPacks.length) {
                        reporter(conglomeratedReport, () => {
                            // here we call the shutdown hooks
                            runHooks('shutdown', (fails) => {
